...
if __name__ == "__main__":
    ...
    parser.add_argument("--analyze-consistency", action="store_true", 
    ...
    parser.add_argument("--show-run", type=str, metavar="TEST_ID",
    ...
    if args.history:
        ...
        print(f"{'Timestamp':<22} | {'Ammeter':<10} | {'Test ID':<38} | {'Status':<6} | {'Pass Rate'}")
        print("-" * 105)
        for r in runs:
            print(f"{r['timestamp']:<22} | {r['ammeter_type']:<10} | {r['test_id']:<38} | {r['status']:<6} | {r['pass_rate']}")
        print("-" * 105 + "\n")
        sys.exit(0)
    ...
    if args.analyze_consistency:
        ...
        print(f"{'Ammeter':<15} | {'Historical Runs':<15} | {'Mean of Means (A)':<20} | {'StdDev/Drift (A)'}")
        print("-" * 75)
        for ammeter, stats in consistency.items():
            print(f"{ammeter.upper():<15} | {stats['historical_runs']:<15} | {stats['mean_of_means']:<20.5f} | {stats['std_dev_of_means']:.5f}")
