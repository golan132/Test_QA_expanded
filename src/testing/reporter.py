...
class ConsoleReporter:
    ...
    def generate_report(result: TestRunResult) -> str:
        ...
        report.append("-" * 30)
        report.append(
            f"Total Attempts: {result.attempted_samples} (Expected: {result.expected_samples})"
        )
        ...
        if result.errors:
            ...
            for e in result.errors[:5]:  # Show up to 5 errors to avoid flooding
                report.append(
                    f"  - [{e.get('timestamp')}] {e.get('error_type')}: {e.get('error_message')}"
                )
    ...
    def generate_report_from_dict(result: dict) -> str:
        ...
        report.append("-" * 30)
        report.append(
            f"Total Attempts: {result.get('attempted_samples')} (Expected: {result.get('expected_samples')})"
        )
