...
def test_analyze_history(tmp_path):
    ...
    file1.write_text(
        json.dumps({"ammeter_type": "greenlee", "statistics": {"mean": 5.0}})
    )
    ...
    file2.write_text(
        json.dumps({"ammeter_type": "greenlee", "statistics": {"mean": 7.0}})
    )
    ...
    file3.write_text(
        json.dumps({"ammeter_type": "entes", "statistics": {"mean": 10.0}})
    )
